...
foreach ($rs_00000001 AS $row_00000001) {
    ...
    $rowCampagna = $dblink->get_row("SELECT id AS id_campagna, id_tipo_marketing, nome, id_prodotto FROM lista_campagne WHERE id = '".ID_CAMPAGNA_RINNOVI_AUTOMATICI."'", true);
    $rowMarketing = $dblink->get_row("SELECT id AS id_tipo_marketing, nome FROM lista_tipo_marketing WHERE id = '".ID_TIPO_MARKETING_RINNOVI."'", true);
    if($row_00000001['abbonamento']=="1"){
        $rowProdotto = $dblink->get_row("SELECT * FROM lista_prodotti WHERE codice_esterno = 'abb_".$row_00000001['id_classe']."'", true);
        if (DISPLAY_DEBUG) echo "<br>".$dblink->get_query();
    }else{
    ...
    if(empty($rowCalendario)){
        ...
        $insert = array(
            "dataagg" => date("Y-m-d H:i:s"),
            "scrittore" => $dblink->filter("autoGeneraRichiesteRinnovi10gg"),
            "datainsert" => date("Y-m-d"),
            "orainsert" => date("H:i:s"),
            "data" => date("Y-m-d"),
            "ora" => date("H:i:s"),
