...
if (isset($_POST['intervallo_data'])) {
    ...
    $where_intervallo = " AND data_creazione BETWEEN  '" . GiraDataOra($data_in) . "' AND  '" . GiraDataOra($data_out) . "'";
    $titolo_intervallo = " dal  " . $data_in . " al  " . $data_out . "";
    //echo '<h1>$intervallo_data = '.$intervallo_data.'</h1>';
} else {
    $where_intervallo = " AND YEAR(data_creazione)=YEAR(CURDATE()) AND MONTH(data_creazione)=MONTH(CURDATE())";
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <div class="input-group" id="defaultrange" name="defaultrange">
                                                <input type="text" class="form-control" id="intervallo_data" name="intervallo_data" onChange="submit();" value="<?=$_POST['intervallo_data']?>">
                                                        ...
                                                        <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="tab1_per_commessa">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="min-phone-l">Entrate &euro;</th>
                                                                    <th class="min-phone-l">Uscite &euro;</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <?php
                                                                $sql_001 = "SELECT
                                                          CONCAT('<a class=\"btn btn-circle btn-icon-only yellow btn-outline\" href=\"" . BASE_URL . "/moduli/costi/dettaglio.php?tbl=lista_costi&id=',id,'\" title=\"DETTAGLIO\" alt=\"DETTAGLIO\"><i class=\"fa fa-search\"></i></a>') AS 'dettaglio',
                                                          data_creazione, entrate, uscite FROM lista_costi WHERE 1 AND data_creazione<=NOW() AND data_creazione!='0000-00-00' " . $where_intervallo . " ORDER BY data_creazione DESC LIMIT 25";
                                                                $rs_001 = $dblink->get_results($sql_001);
                                                                if (!empty($rs_001)) {
                                                                    foreach ($rs_001 as $row_001) {
                                                                        echo '<tr>
                                                                            <td>' . $row_001['dettaglio'] . '</td>
                                                                            <td>' . GiraDataOra($row_001['data_creazione']) . '</td>
                                                                            <td>' . $row_001['entrate'] . '</td>
                                                                          <td>' . $row_001['uscite'] . '</td>
                                                                        </tr>';
                                                                    }
                                                                }
                                                                ?>
