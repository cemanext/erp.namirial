...
switch($tabella){
    case "lista_ticket":
        $campi_visualizzati = $table_tickets['index']['campi'];
        $where = $table_tickets['index']['where'];
        if(!empty($arrayCampoRicerca)){
            foreach ($arrayCampoRicerca as $campoRicerca) {
                ...
                $where.= " OR datainsert LIKE '%".$campoRicerca."%'";
                $where.= " OR data_fine LIKE '%".$campoRicerca."%' OR stato LIKE '%".$campoRicerca."%')";
            }
        }
        $ordine = $table_tickets['index']['order'];
    break;
