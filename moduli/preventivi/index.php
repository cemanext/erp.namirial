...
if(isset($_GET['whr_state']) && $_GET['whr_state']!="null" && $_GET['whr_state']!=""){
    if (!empty($_GET['intervallo_data_preventivi'])) {
        $intervallo_data_preventivi = $_GET['intervallo_data_preventivi'];
        $_SESSION['intervallo_data_preventivi'] = $_GET['intervallo_data_preventivi'];
        $data_in = before(' al ', $intervallo_data_preventivi);
        $data_out = after(' al ', $intervallo_data_preventivi);
        
        $_GET['intervallo_data_preventivi'] = ($data_in)." al ".($data_out);
        ...
        if("01-".date("m-Y")." al ".date("t-m-Y") == $intervallo_data_preventivi){
            $titolo_intervallo = " del mese in corso";
        }else if(date("d-m-Y", strtotime("-29 days"))." al ".date('d-m-Y') == $intervallo_data_preventivi) {
            $titolo_intervallo = " utlimi 30 gioni";
        }else if(date("d-m-Y", strtotime("-6 days"))." al ".date('d-m-Y') == $intervallo_data_preventivi) {
            $titolo_intervallo = " utlimi 7 gioni";
        }else if(date("d-m-Y", strtotime("-1 days"))." al ".date('d-m-Y', strtotime("-1 days")) == $intervallo_data_preventivi) {
            $titolo_intervallo = " ieri";
        }elseif(date("d-m-Y")." al ".date('d-m-Y') == $intervallo_data_preventivi) {
            $titolo_intervallo = " oggi";
        }else{
            $titolo_intervallo = " dal  " . $data_in . " al  " . $data_out . "";
        }
        //echo '<h1>$intervallo_data_preventivi = '.$intervallo_data_preventivi.'</h1>';
    } else {
        ...
        $titolo_intervallo = " del mese in corso";
        $_SESSION['intervallo_data_preventivi'] = GiraDataOra($richiestaAperta['data'])." al ".date("d-m-Y");
        $_GET['intervallo_data_preventivi'] = GiraDataOra($richiestaAperta['data'])." al ".date("d-m-Y");
    ...
    $_SESSION['intervallo_data_preventivi'] = null;
    unset($_SESSION['intervallo_data_preventivi']);
}

$sql_007 = "UPDATE `lista_preventivi`
SET `codice_interno`=`id`
WHERE `codice_interno`<=0";
$rs_007 = $dblink->query($sql_007);

$sql_007 = "UPDATE `lista_preventivi`, `lista_preventivi_dettaglio` 
SET `lista_preventivi_dettaglio`.`sezionale`=`lista_preventivi`.`sezionale`
WHERE `lista_preventivi`.`id`=`lista_preventivi_dettaglio`.`id_preventivo`
AND LENGTH(`lista_preventivi`.`sezionale`)>0";
$rs_007 = $dblink->query($sql_007);
?>
<!DOCTYPE html>
        ...
        <div class="page-container">
            ...
            <div class="page-content-wrapper">
                ...
                <div class="page-content">
                    ...
                    <!-- END PAGE BAR -->
                    <?php if(isset($_GET['whr_state']) && $_GET['whr_state']!="null" && $_GET['whr_state']!=""){ ?>
                    <div class="clearfix"></div>
                    <div class="row" style="margin-top: 10px; margin-bottom: -20px;">
                        <form action="?" class="form-horizontal form-bordered" method="GET" id="formIntervallo" name="formIntervallo">
                            <input type="hidden" name="whr_state" id="whr_state" value="<?=$_GET['whr_state']?>">
                                ...
                                <div class="form-body">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group" id="dataRangeHome" name="dataRangeHome">
                                            <input type="text" class="form-control" id="intervallo_data_preventivi" name="intervallo_data_preventivi" value="<?=$_GET['intervallo_data_preventivi']?>" readonly="true">
                    ...
                    <!-- END PAGE TITLE -->
                    <!-- END PAGE HEADER-->
                    <?php Stampa_HTML_index_Preventivi($tabella); ?>
                    <div class="form-actions right">
                        <a href="modifica.php?tbl=<?=$tabella?>&id=0" type="submit" class="btn btn-circle btn-lg green-jungle"><i class="fa fa-plus"></i> Aggiungi Nuovo</a>
                    </div>
                </div>
                <!-- END CONTENT BODY -->
            </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                function (startDate, endDate) {
                    $('#intervallo_data_preventivi').val(startDate.format('DD-MM-YYYY') + ' al ' + endDate.format('DD-MM-YYYY'));
                ...
                $('#intervallo_data_preventivi').on('change', function(ev, picker) {
                    document.formIntervallo.submit();
                });
