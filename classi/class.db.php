...
class DB
{
    ...
    {
        $this->log->log_db_errors($error, $query, "ERRORE");
    ...
    /**
     * Return specific filed value based on db query
     *
     * Example usage:
     * $name = $database->get_field( "SELECT name FROM users WHERE user_id = 44" );
     *
     * @access public
     * @param string
     ...
     *
     */
    public function get_field( $query, $array = false )
    {
        self::$counter++;
    ...
    public function get_row( $query, $object = false )
    {
        ...
        {
            $r = ( !$object ) ? $row->fetch_row() : $row->fetch_assoc();
            return $r;   
        }
    }
    ...
    */
    ...
    public function update( $table, $variables = array(), $where = array(), $limit = '' )
    {
        ...
        foreach( $variables as $field => $value )
        {
            if(strpos($value, "CONCAT(")!==false || $value === "NOW()") { //|| strpos($value, "NOW()")!==false
                $updates[] = "`$field` = $value";
            }else{
                $updates[] = "`$field` = '$value'";
            }
     ...
     *
     */
    public function updateWhere( $table, $variables = array(), $where = '', $limit = '' )
    {
        self::$counter++;
        //Make sure the required data is passed before continuing
        //This does not include the $where variable as (though infrequently)
     ...
     *
     */
    public function duplicateWhere( $table, $where = "", $limit = '', $orderby = '')
    {
        self::$counter++;
        //Make sure the required data is passed before continuing
        //This does not include the $where variable as (though infrequently)
        ...
        foreach ($res as $value) {
            foreach($value as $key => $row) {
                switch ($key) {
                    ...
                    case "scrittore":
                        $insert[$key]=$this->filter($_SESSION['cognome_nome_utente']);
                    break;
     ...
     * @return array
     */
    public function list_fields( $query , $resultSet=false )
    {
        self::$counter++;
        if($resultSet){
